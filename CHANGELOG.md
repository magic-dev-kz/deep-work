...
- **Performance: deferred non-critical init**: Heatmap rendering, productivity insight, time tracking dashboard, and SW registration wrapped in `requestIdleCallback` (with `setTimeout` fallback) so main UI renders before secondary features load
- SW cache bumped to `deepwork-v26`
...
- SW cache bumped to `deepwork-v25`
...
- SW cache bumped to `deepwork-v24`
